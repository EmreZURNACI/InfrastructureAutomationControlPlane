...
func (srv *EbsService) Attach(c *fiber.Ctx) error {
	var req ebs.AttachRequest
	...
	_, err := srv.AttachService.Execute(c.UserContext(), &req)
	if err != nil {
		return c.JSON(controllers.FailureResponse(400, err.Error(), err.Error()))
	}

	return c.JSON(controllers.SuccessResponse(200, "the block device attached the instance successfully", nil))
}
